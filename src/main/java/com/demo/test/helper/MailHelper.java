...
public class MailHelper {
     ...
     */
    public static void sendSimpleEmail(String host, String username, String password,
                                       String subject, String contents, String toEmailAddress,
    ...
    public static void sendSimpleEmail(String host, String username, String password,
        ...
        email.setAuthentication(username, password);
        //发送给多个人
        for (int i = 0, toSize = toEmailAddress.length; i < toSize; i++) {
    ...
    public static void sendMultiPartEmail(String host, String username, String password, String subject,
        ...
        if (multiPaths != null) {
            for (int i = 0, size = multiPaths.length; i < size; i++) {
        ...
        for (int i = 0, attachSize = attachmentList.size(); i < attachSize; i++) {
            email.attach(attachmentList.get(i));
        }
        email.send();
    }
    ...
    public static void sendMultiPartEmail(String host, String username, String password, String subject,
        ...
        if (multiPaths != null) {
            for (int i = 0; i < multiPaths.length; i++) {
                EmailAttachment attachment = new EmailAttachment();
                //判断当前这个文件路径是否在本地, 是：setPath,否则 setURL
                if (multiPaths[i].indexOf("http") == -1) {
                    attachment.setPath(multiPaths[i]);
                } else {
                    attachment.setURL(new URL(multiPaths[i]));
        ...
        email.setSubject(subject);
        email.setMsg(contents);
        for (int i = 0, attSize = attachmentList.size(); i < attSize; i++) {
            email.attach(attachmentList.get(i));
        }
        email.send();
    }
     ...
     * @throws EmailException
     */
    public static void sendHtmlEmail(String host, String username, String password,
                                     String subject, String contents, String toEmailAddress,
                                     String fromEmailAddress) throws EmailException {
